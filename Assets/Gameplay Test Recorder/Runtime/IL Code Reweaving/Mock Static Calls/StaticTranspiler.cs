...
{
    internal static class StaticTranspiler
    {
        public readonly static MethodInfo transpiler;
        public static IReadOnlyList<StaticMock> mockedTypes;

        static StaticTranspiler()
        {
            transpiler = typeof(StaticTranspiler).GetMethod("Transpiler", BindingFlags.Static | BindingFlags.Public);
            Assert.IsNotNull(transpiler);
        }

        public static bool IsMockedMethod(Type type, MethodInfo mi)
        {
            foreach (var mock in mockedTypes)
            {
                if (mock.Callee.Equals(type))
                {
                    return mock.IsMockedMethod(mi);
                }
            }
            return false;
        }
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator, MethodBase method)
        {
            ...
            return ReplaceCalls(instructions, InsertSwitch, generator, methodKey);
        }
        ...
        private static LinkedListNode<CodeInstruction> CreateRecordCase(InsertCallData data, Label recordLabel, Label switchEndLabel, LinkedListNode<CodeInstruction> defaultBreak, string key)
        {
            ...
            // add record call
            CodeInstruction recordInstruction = CodeInstruction.Call("TwoGuyGames.GTR.Core.ValueRecorder:Store", generics: new[] { data.CalledMethod.ReturnType });
            LinkedListNode<CodeInstruction> recordCall = data.Instructions.AddAfter(recordKeyNode, recordInstruction); // Add the recording call after the call to the input system. This will put the value read from the input system back on the stack via return.
            LinkedListNode<CodeInstruction> recordBreak = AddGotoLabel(data.Instructions, recordCall, switchEndLabel);
            return recordBreak;
        }

        private static LinkedListNode<CodeInstruction> CreateReplayCase(InsertCallData data, string methodKey, Label replayLabel, Label switchEndLabel, LinkedListNode<CodeInstruction> recordBreak)
        {
            // pop all arguments
            int parameterCount = data.CalledMethod.GetParameters().Length;
            LinkedListNode<CodeInstruction> addAfter = recordBreak;
            for (int i = 0; i < parameterCount; i++)
            {
                CodeInstruction popArg = new CodeInstruction(OpCodes.Pop);
                addAfter = data.Instructions.AddAfter(addAfter, popArg);
            }
            LinkedListNode<CodeInstruction> replayKeyNode = data.Instructions.AddAfter(addAfter, new CodeInstruction(OpCodes.Ldstr, methodKey)); // push key argument onto stack
            CodeInstruction replayInstruction = CodeInstruction.Call("TwoGuyGames.GTR.Core.ValueRecorder:NextInput", generics: new[] { data.CalledMethod.ReturnType });
        ...
        /// </summary>
        private static LinkedListNode<CodeInstruction> InsertSwitch(InsertCallData data)
        {
            if (data.CalledMethod.ReturnType != typeof(void))
            {
                ...
                LinkedListNode<CodeInstruction> recordBreak = CreateRecordCase(data, recordLabel, switchEndLabel, defaultBreak, data.MethodKey);
                LinkedListNode<CodeInstruction> replayBreak = CreateReplayCase(data, data.MethodKey, replayLabel, switchEndLabel, recordBreak);
            ...
            return data.InputCall;
        }

        private static IEnumerable<CodeInstruction> ReplaceCalls(IEnumerable<CodeInstruction> instructions, InsertCallData.InsertCall modify, ILGenerator generator, string methodKey)
        {
            bool log = false;
            //log = methodKey.Contains("GraphicRaycaster.Raycast");
            if (log)
                UnityEngine.Debug.Log("     " + methodKey + " TRANSPILE");
            LinkedList<CodeInstruction> instrList = new LinkedList<CodeInstruction>(instructions);
            if (log)
                UnityEngine.Debug.Log(instrList.Count);
            ...
            while (current != null)
            {
                ...
                if (ReflectionHelper.IsMethodCall(current.Value) &&
                    current.Value.operand is MethodInfo method &&
                    method.IsStatic &&
                    IsMockedMethod(method.DeclaringType, method) &&
                    RecordFactory.IsSupportedType(method.ReturnType))
                {
