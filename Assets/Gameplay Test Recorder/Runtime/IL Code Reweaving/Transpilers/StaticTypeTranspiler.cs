...
{
    /// <summary>
    /// Replaces calls to any given static class with recording/replaying calls.
    ...
    /// </summary>
    public static class StaticTypeTranspiler
    {
        public static readonly MethodInfo transpiler;
        public static Type staticTypeToReplace;

        static StaticTypeTranspiler()
        {
            transpiler = typeof(StaticTypeTranspiler).GetMethod("Transpiler", BindingFlags.Static | BindingFlags.Public);
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator, MethodBase method)
        {
            ...
            return ReplaceCalls(instructions, InsertSwitch, transpiler, generator, methodKey);
        }
        ...
        private static IEnumerable<CodeInstruction> ReplaceCalls(IEnumerable<CodeInstruction> instructions, InsertCallData.InsertCall callModifier, MethodInfo transpiler, ILGenerator generator, string methodKey)
        {
            ...
            while (current != null)
            {
                ...
                if (ReflectionHelper.IsMethodCall(current.Value) && current.Value.operand is MethodInfo method)
                {
                    if (method.DeclaringType == staticTypeToReplace)
                    {
                        data.CalledMethod = method;
                        data.InputCall = current;
                        current = callModifier(data);
                    }
                }
